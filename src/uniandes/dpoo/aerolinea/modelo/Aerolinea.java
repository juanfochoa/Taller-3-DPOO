...
public class Aerolinea
{
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        if (ruta == null || avion == null) {
            throw new Exception("error sapos");
        }

        // Verificar que el avión no esté programado en otro vuelo en la misma fecha
        for (Vuelo vuelo : vuelos) {
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        if (vuelo.getTiquetes().size() + cantidad > ((Avion) vuelo.getAvion()).getCapacidad()) {
            throw new VueloSobrevendidoException(vuelo);
        }

        CalculadoraTarifas calculadora = new CalculadoraTarifas(); // Se asume que existe esta clase
    ...
    public String consultarSalgoPendienteCliente(String identificadorCliente) {
    	Cliente cliente = getCliente(identificadorCliente);
    	if(cliente == null) {
    		return "No se encontró el cliente";
    	}
    	int saldoPendiente = cliente.calcularValorTotalTiquetes();
    	return "El salgo pendiente es: " + saldoPendiente;
    }
